...
- Для загрузки на FTPS используется `aioftp` (устанавливается из `requirements.txt`)

## Быстрый старт (Windows, one-click)
1. Запустить веб-интерфейс:
   ```bash
   start_web_ui.bat
   ```
2. Скрипт автоматически:
   - проверит Python в PATH;
   - создаст `.venv` при отсутствии;
   - обновит `pip`;
   - установит/обновит зависимости из `requirements.txt`;
   - создаст `.env` из `.env.example` (если `.env` отсутствует);
   - запустит веб-интерфейс.
3. В веб-форме заполнить:
   - `API_ID`
   - `API_HASH`
...
Альтернативно, только установка/обновление зависимостей:
```bash
...
- Кнопка `Прервать загрузку`:
  - останавливает только текущую загрузку, не выключая веб-интерфейс
- Чекбокс `Дебаг`:
  - открывает встроенную консоль внизу страницы
  - показывает live-логи приложения для диагностики ошибок
  - при включении сохраняет расширенный лог в файл `data/debug.log.txt` (или `/app/data/debug.log.txt` в Docker)
...
- Действия формы (`Авторизоваться`, `Проверить SFTP/FTPS`, `Обновить предпросмотр`, `Старт/Стоп`) выполняются через `fetch` без перезагрузки страницы и без перехода по URL
...
- `FTPS_UPLOAD_CONCURRENCY` - количество параллельных upload-потоков для кнопки "Начать загрузку" (по умолчанию `2`)
