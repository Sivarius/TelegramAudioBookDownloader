...
def persist_runtime_state(deps: dict, form: dict, settings) -> None:
...
def validate_stack(deps: dict, form: dict, settings, include_proxy: bool):
    def _validate_proxy():
    ...
    if include_proxy:
        ok, response = _validate_proxy()
        if not ok:
            return False, response
    ok, response = _validate_sftp()
    if not ok:
        return False, response
    ok, response = _validate_ftps()
    if not ok:
        return False, response
    return True, None


def handle_authorize(deps: dict, form: dict):
    try:
        settings = deps["build_settings"](form)
    except ValueError as exc:
        return deps["render"](form, str(exc))

    persist_runtime_state(deps, form, settings)
    runtime_state = deps["runtime_state"]
    if not runtime_state["remember_me"]:
        deps["delete_session_files"](settings.session_name)

    ok, response = validate_stack(deps, form, settings, include_proxy=True)
    if not ok:
        return response
...
def handle_start_download(deps: dict, form: dict):
    try:
        settings = deps["build_settings"](form)
    except ValueError as exc:
        return deps["render"](form, str(exc))
...
def handle_start_upload(deps: dict, form: dict):
    try:
    ...
    ok, response = validate_stack(deps, form, settings, include_proxy=False)
    if not ok:
        return response
...
def handle_stop_server(deps: dict, shutdown_func):
...
def handle_stop_download(deps: dict, form: dict):
    worker_thread = deps["get_worker_thread"]()
